...
async function generate({ entryPoints = [], inDir, outDir, typedoc, ignoreErrors = false}: Options, typeDocOptions: Partial<TypeDocOptions> = {}) {
  ...
  const typedocConfigFile = typedoc ? (await import('./' + typedoc))?.default.default || {} : {};
  ...
  const project = ignoreErrors ? app.converter.convert(app.getEntryPoints() ?? []) : app.convert();
...
async function start() {
  ...
  await generate({inDir: argv.in, outDir: argv.out, typedoc: argv.options, ignoreErrors: argv.ignoreErrors}, {});
}
